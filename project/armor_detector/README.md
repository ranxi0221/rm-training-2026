...
## W7 改进记录（2026-06-19）：数字识别模型接入

### 模型信息

| 项目 | 值 |
|------|-----|
| 模型文件 | `models/tiny_resnet.onnx`（OpenCV DNN 加载） |
| 输入尺寸 | 32×32 单通道灰度图 |
| 归一化 | [0, 1]（像素值 / 255） |
| 输出类别 | 9 类：1, 2, 3, 4, 5, sentry, outpost, base, not_armor |
| 预处理 | 灰度化 → 等比例缩放 → 黑底填充 → 归一化 |

### ROI 提取方法

从装甲板四角外接矩形中心裁切：
- **长方形装甲板**（宽高比 > 1.2）：取中心 60% 宽 × 85% 高
- **正方形装甲板**（宽高比 ≤ 1.2，有图标干扰）：取中心 35% 宽 × 85% 高

### BGR 颜色纯度过滤（核心改进）

HSV 分离后增加 BGR 通道比例验证，过滤白色/灰色表面：

| 颜色 | 条件 |
|------|------|
| 蓝色 | B > 120 且 B > R × 1.7 且 B > G × 1.7 |
| 红色 | R > 120 且 R > B × 1.7 且 R > G × 1.7 |

> 原理：白墙 B≈G≈R，比例约 1.0；真 LED 目标通道远超其他通道。

### 新增/修改文件

| 文件 | 说明 |
|------|------|
| `include/armor_detector/digit_classifier.hpp` | 数字分类器类声明 |
| `src/digit_classifier.cpp` | 分类器实现（ROI提取 + ONNX推理） |
| `models/tiny_resnet.onnx` | 数字识别模型 |
| `src/armor_detector.cpp` | BGR纯度过滤、灯条尺寸配对、帧间数字记忆、翻转文字标签 |
| `src/armor_detector_node.cpp` | 模型延迟加载、相机180°翻转 |
| `CMakeLists.txt` | 添加 digit_classifier 编译 |
| `launch/armor.launch.py` | `digit_model_path` 参数 |
| `config/armor_params.yaml` | 最终暗环境参数 |

### 识别结果可视化

标签格式：`blue_1`、`red_1`，翻转文字贴于装甲板左上角。

截图：[project/armor_detector/assets/red1,blue1/]

### 错误样例分析

| # | 现象 | 根因 | 解决方向 |
|:--:|------|------|------|
| 1 | 暗环境数字不可见 | 曝光极低，印刷数字无光照 | 需适度提高曝光或外置补光 |
| 2 | 数字偶尔闪烁丢失 | 灯条检测不稳定导致装甲板漏检 | 帧间数字记忆保留 30 帧 |
| 3 | 正方形装甲板图标干扰 | 图标白色边框与数字同色 | 中心窄裁切 35% 避开图标 |
| 4 | 亮环境误检增多 | HSV 阈值针对暗环境调优 | BGR 纯度过滤 + 亮环境模式 |

### W7 最终参数

```yaml
# 暗环境核心参数（config/armor_params.yaml）
red1_s_low: 30, red1_v_low: 100
blue_s_low: 40, blue_v_low: 150, blue_h: 100-125
light_min_area: 20, light_max_ratio: 0.6
pair_max_dy: 400, pair_min_dx: 5, pair_max_dx: 2500
smooth_alpha: 0.4
```
